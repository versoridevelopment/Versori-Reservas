...
// ✅ FIX Next.js 15 types: params llega tipado como Promise
...
export async function PATCH(req: Request, ctx: RouteContext) {
  try {
    ...
    if (body.id_cancha !== undefined)
      patch.id_cancha = body.id_cancha ? Number(body.id_cancha) : null;

    if (body.fecha !== undefined) {
      if (!body.fecha || !isISODate(body.fecha))
    ...
    if (body.cierre_total === true) {
      ...
      const wantsTimeEdit =
        ...
        body.fin !== undefined ||
        body.cruza_medianoche !== undefined;
      ...
      if (wantsTimeEdit) {
        ...
        const fin_dia_offset =
          body.cruza_medianoche !== undefined
            ? body.cruza_medianoche
              ? 1
              : 0
            : current.fin_dia_offset;

...
type TipoTurno = "normal" | "profesor" | "torneo" | "escuela" | "cumpleanos" | "abonado";
...
export async function POST(req: Request) {
  try {
    ...
    const duracion_min = Number(body?.duracion_min || 0) as 60 | 90 | 120;

    const tipo_turno = String(body?.tipo_turno || "normal") as TipoTurno;
    const notas = body?.notas ?? null;

    const cliente_nombre = (body?.cliente_nombre ?? "").toString();
    const cliente_telefono = (body?.cliente_telefono ?? "").toString();
    const cliente_email = (body?.cliente_email ?? "").toString();

    if (!id_club || Number.isNaN(id_club)) return NextResponse.json({ error: "id_club requerido" }, { status: 400 });
    if (!id_cancha || Number.isNaN(id_cancha)) return NextResponse.json({ error: "id_cancha requerido" }, { status: 400 });
    ...
    if (![60, 90, 120].includes(duracion_min)) {
      return NextResponse.json({ error: "duracion_min inválida (60/90/120)" }, { status: 400 });
    }
    ...
    if (!adminUserId) return NextResponse.json({ error: "LOGIN_REQUERIDO" }, { status: 401 });

    // Permisos
    const perm = await assertAdminOrStaff({ id_club, userId: adminUserId });
    if (!perm.ok) return NextResponse.json({ error: perm.error }, { status: perm.status });

    // Validación mínima cliente (ajustable)
    if (cliente_nombre.trim() === "" && cliente_telefono.trim() === "") {
      return NextResponse.json({ error: "Cliente requerido (nombre o teléfono)" }, { status: 400 });
    }

    // Calcular fin + fin_dia_offset
    ...
    // Segmento por tipo_turno (regla simple)
    const segmento_override: Segmento = tipo_turno === "profesor" ? "profe" : "publico";

    // 1) Calcular precio (reutiliza tu endpoint)
    ...
    if (!Number.isFinite(precio_total) || precio_total <= 0) {
      return NextResponse.json({ error: "Precio inválido calculado" }, { status: 409 });
    }
    ...
    // 2) Anticipo del club (si querés guardar snapshot del anticipo)
    ...
    // 3) Insert reserva confirmada (manual admin) — anti-solape lo garantiza el constraint
    const { data: inserted, error: insErr } = await supabaseAdmin
      ...
      .insert({
        ...
        expires_at: null, // manual => confirmada
      })
      .select("id_reserva")
      .single();

    if (insErr) {
      // EXCLUDE constraint => Postgres 23P01 (exclusion_violation)
      if ((insErr as any).code === "23P01") {
        return NextResponse.json({ error: "TURNOS_SOLAPADOS" }, { status: 409 });
      }
      return NextResponse.json({ error: `Error creando reserva: ${insErr.message}`, detail: insErr }, { status: 500 });
    }

...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!id_reserva) return NextResponse.json({ error: "ID inválido" }, { status: 400 });

    // 1. Auth (Seguridad básica)
    const supabase = await getSupabaseServerClient();
    const { data: authRes } = await supabase.auth.getUser();
    if (!authRes?.user?.id) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    ...
    const cliente_nombre = (reserva.cliente_nombre || 
        (reserva.profile ? `${reserva.profile.nombre} ${reserva.profile.apellido}` : "")).trim() || "Sin nombre";
    ...
    const saldo = Math.max(0, (reserva.precio_total || 0) - pagosAprobados);
    const pagado_total = saldo === 0 && (reserva.precio_total || 0) > 0;
